...
export default function PedidosPage() {
  ...
  const handleRequestConfirm = (orderId: string) => {
    setPendingConfirmationId((current) => (current === orderId ? null : orderId));
  };
      ...
      <main className="container mx-auto px-4 py-10">
        <div className="mx-auto max-w-5xl">
          <div className="mb-8 space-y-2">
            <p className="text-sm uppercase tracking-[0.3em] text-muted-foreground">Meus pedidos</p>
            <h1 className="text-3xl font-bold text-foreground">Histórico de compras</h1>
          ...
          {loading ? (
            ...
            <div className="rounded-3xl border border-border bg-secondary p-10 text-center">
              ...
              <h2 className="text-2xl font-semibold text-foreground">Nenhuma compra realizada ainda</h2>
                ...
                <Card key={order.id} className="overflow-hidden">
                  <CardHeader className="space-y-4 border-b border-border px-6 py-4">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {order.status !== 'delivered' && (
                          <Button variant="outline" size="sm" onClick={() => handleRequestConfirm(order.id)}>
                            Confirmar entrega
                          </Button>
                        )}
                      </div>
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <div className="rounded-xl border border-border bg-secondary/80 p-4">
                        <p className="text-xs uppercase tracking-[0.2em] text-muted-foreground">Pagamento</p>
                        <p className="mt-2 font-medium text-foreground">
                      ...
                      <div className="rounded-xl border border-border bg-secondary/80 p-4">
                        ...
                        <p className="mt-2 text-lg font-semibold text-primary">{formatPrice(order.total)}</p>
                      </div>
                    </div>
                  </CardHeader>
                    ...
                    {pendingConfirmationId === order.id && (
                      <div className="rounded-xl border border-destructive/20 bg-destructive/10 p-4 text-sm text-destructive">
                        ...
                        <div className="mt-3 flex flex-wrap gap-3">
                          <Button variant="default" size="sm" onClick={() => handleConfirmDelivery(order.id)} disabled={statusLoading[order.id]}>
                            {statusLoading[order.id] ? 'Confirmando...' : 'Sim, confirmar entrega'}
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => setPendingConfirmationId(null)}>
                            Cancelar
                          </Button>
                        </div>
                      </div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-3">
                          {order.items.map((item: any, index: number) => (
                            <div key={`${order.id}-${index}`} className="rounded-2xl border border-border bg-background p-4">
                              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                <div>
                              ...
                              {item.personalization ? (
                                <p className="mt-2 text-sm text-muted-foreground">Personalização: {item.personalization}</p>
                              ) : null}
                              <p className="mt-2 text-sm text-muted-foreground">Valor unitário: {formatPrice(item.unitPrice)}</p>
                            </div>
                          ))}
                        </div>
                      </div>

                      <div className="space-y-4 rounded-2xl border border-border bg-secondary/80 p-4">
                        <div>
                          <div className="flex items-center gap-2 text-sm text-muted-foreground uppercase tracking-[0.2em]">
                          ...
                          {order.deliveryMethod === 'delivery' && order.address ? (
                            <div className="mt-3 space-y-2 text-sm text-muted-foreground">
                              ...
                              {order.address.complement ? <p>{order.address.complement}</p> : null}
                              <p>{order.address.neighborhood}</p>
                              <p>{order.address.city} - {order.address.state}</p>
                              <p>CEP: {order.address.cep}</p>
                            </div>
                          ) : (
                            <p className="mt-3 text-sm text-muted-foreground">Retirada diretamente no campus.</p>
                          )}
                        </div>
                        ...
                        <div>
                          ...
                          <p className="mt-3 text-sm text-foreground">{paymentLabel[order.paymentMethod] || order.paymentMethod}</p>
                        </div>
